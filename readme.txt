...
When results of parameter estimation are compared to the output of arima() command in R, remember that the sign of moving average parameters must be differ. In R the ARMA(1,1) model, for example, is z_t=phi*z_(t-1) + pheta*a_(t-1). Here the model is z_t=phi*z_(t-1)-pheta*a_(t-1).
